...
void Engine::WinScreen::Init(Game* game) {
    ...
    playSprite->AddAnimation("normal", 3, 3);
    playSprite->AddAnimation("hover", 2, 3);
    playSprite->AddAnimation("press", 2, 3);
    ...
    text->SetText("Thanks For Play the Game");
    text->SetPosition((setting->screenWidth / 2) - 780, setting->screenHeight - 300);
    text->SetColor(250, 249, 246);

    // Create Text
    text2 = new Text("Retro.ttf", 100, game->GetDefaultTextShader());
    text2->SetText("Press Retry to Play Again");
    text2->SetPosition((setting->screenWidth / 2) - 780, setting->screenHeight - 450);
...
void Engine::WinScreen::Update(Game* game) {
    // Set background
...
void Engine::WinScreen::Render(Game* game) {
    //render background
    BGSprite->Draw();
    // Render all buttons
    for (Button* b : buttons) {
